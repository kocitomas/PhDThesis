...
The generic model of a flexible, elastic, homopolymer is suitable for the investigation of the thermodynamic properties of polymer chains on a coarse-grained level. The individual monomers have a neutral electric charge and do not interact via Coulomb forces. Instead, structure forming processes are primarily driven by effective dipole-dipole interactions represented by the van Der Waals forces. 
...
\end{equation}
where $\mathbf{p}_{i}$ is the dipole moment of the $i$th monomer and $\hat{\mathbf{r}}$ is the unit vector in the direction of the separation vector between the monomers. The strength of the potential decreases with the third power of the distance, however this is not consistent with the $1/r^{6}$ decay which has been observed in experiments. In fact, the problem must be treated quantum mechanically to account for the existing overlap between electron wave functions. The potential $V_{\mathrm{dip}}(r)$ is replaced by a dipole-dipole operator $\hat{H}_{\mathrm{dip}}$, which is then introduced as a perturbation to a system of two non-interacting monomers. The first non-trivial term in the perturbation expansion of the ground state energy of the system yields the desired $1/r^{6}$ dependence. In addition to this generic long range attraction, interacting bodies experience strong short range repulsion due to repelling electronic clouds.  Both effects are contained in the famous Lennard-Jones potential
%
\begin{equation}
...
\end{equation}
%
where $\epsilon$  sets the energy scale of the interaction while the relevant length scale is given by the van der Walls distance $\sigma$. 

longitudinal bond vibrations
elastic bond potential
harmonic potential 
FENE potential
asymmetric contribution of LJ
represent covalent bonds in a generic fashion
intended for the investigation of structural phases of polymers on coarse-grained level
...
where
 ...
 We select a cut-off radius $r_c=2.5\sigma$ such that
