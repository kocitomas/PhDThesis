...
\begin{figure}
\center
	\includegraphics[width=0.9\textwidth]{chapter6Figs/configurationsMod.eps}			\caption{\label{fig:coreStructures} 
	(a) Icosahedral cores are commonly found in the ground states of short polymer chains. (b) The unusual bihexagonal core is found in polymer chains of length $N =15,55$ with sufficiently weak bonded Lennard-Jones interactions.}
\end{figure}
%

%
Using $\sim 10^{6}$ polymer structures per value of $\eta$, we have computed $Q_{l}$ up to $l = 8$ and found that $Q_{6}$ is most effective in resolving the six-fold dihedral symmetry of the bihexagon and the icosahedral symmetry. For a perfect icosahedral core, $Q_{6} \approx 0.65$, whereas a bihexagonal core corresponds to $Q_{6} \approx 0.41$. The results are presented in the form of intensity plots in Figs.~\ref{fig:intensityPlots15} and~\ref{fig:intensityPlots55}, for the 15mer and the 55mer, respectively. The probability of detecting a conformation with a specific value of the order parameter $Q_{6}$ at an energy $E$ is represented by shading; black indicating the zero probability and red corresponding to the maximum probability.
%
