...
In this study, we employ a coarse-grained model of an elastic, flexible,
linear homopolymer chain with $N$ monomers, in the same form as introduced in Sec.\,\,\ref{subsec:ElasticChain}. The energy and the length scales of the truncated and shifted Lennard-Jones potential (Eq.\,\,\ref{eq:LJTruncated}) were set to 
$\epsilon=1$ and $\sigma=r_0/2^{1/6}$ respectively, where $r_0 = 0.7$ is the location of the minimum potential. The cut-off radius was set at $r_c=2.5\sigma$. The FENE potential (Eq.\,\,\ref{eq:FENE}) in the bonded interactions is used here in the modified form
%
\begin{equation}
\label{eq:ModFENE}
...
\end{equation}
%
where $K=40$ and $R_{0}=0.3$~\cite{Gross2013}. The parameter $R$ controls the effective elasticity of the polymer bonds, whereas the energy scale is kept constant. The qualitative behavior of the combined bond potential for different values of $R$ is shown in Fig.~\ref{fig:ModifiedBondedPotential}. The bond elasticity increases with $R$, i.e., by changing $R$ in a wide range of values ($R\in[0.3,90]$), we systematically investigate an entire class of polymer systems between the limits of stiff polymers ($R\to 0$) and, effectively, a gas of nonbonded Lennard-Jones particles, for which $R\to \infty$.  
...
\begin{figure}
\center
\includegraphics[width = 0.7\textwidth]{chapter5Figs/bond_potential.eps}
\caption{\label{fig:ModifiedBondedPotential}%
Behavior of the combined bond potential $U_{\mathrm{total}}(r_{ii+1}) = U_{\mathrm{LJ}}(r_{ii+1}) + U_{\mathrm{FENE}}(r_{ii+1})$ for different values of the effective bond elasticity $R$.}
\end{figure} 
