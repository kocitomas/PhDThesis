...
%%%%%%%%%% 	     CHAPTER 6				   %%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Impact of Bonded Interactions on the Thermodynamic and Structural Properties of Flexible Homopolymers}

Bonded interactions in coarse-grained models of elastic polymers are commonly represented by the symmetric, finitely extensible nonlinear elastic (FENE) potential. In chapter \ref{chap:HomopolymerModel}, we have introduced an alternative model which contained an additional Lennard-Jones (LJ) term in the bonded potential. As a result, the bonded potential becomes strongly repulsive at close range and the symmetry of the FENE potential is broken. In this chapter, we systematically investigate the effects of the width and asymmetry of the bonded potential on the structural and thermodynamic properties of short polymer chains. Utilizing the methods of canonical and microcanonical analysis we construct hyperphase diagrams for chains of length $N = 15,55$. Careful investigation of the geometries of low energy conformations is carried out with the help of structural order parameters which were introduced in section \ref{sec:orderParameters}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%%%%%%%%%% 	     MODEL AND METHODS		   %%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Model and Simulation Parameters}
The coarse-grained model of an elastic homopolymer has already been described in chapter \ref{chap:HomopolymerModel}.Non-bonded interactions are represented by the truncated and shifted Lennard-Jones (LJ) potential. The energy scale is set to $\epsilon=1$ and the van-der-Waals radius to $\sigma=r_0/2^{1/6}$, where $r_0 = 1.0$ is the location of the potential minimum. We select a cut-off radius at $r_c=2.5\,\sigma$ and introduce a shift $U_{\mathrm{LJ}}(r_{c}) \approx -0.0163169$, to avoid discontinuities in the potential.

In order to investigate the properties of the model over a whole range of widths and shapes of the bonded potential, it is introduced here in the modified form:
%
\begin{equation}
\label{eq:BondedInteractionModified}
U_{\mathrm{bonded}}(r_{ii+1}) = U_{\mathrm{FENE}}(r_{ii+1}) + \eta [U_{\mathrm{LJ}}(r_{ii=1}) + \epsilon] - [\epsilon + U_{\mathrm{LJ}}(r_{c})].
\end{equation}
%
The first term consists of the anharmonic FENE potential in its standard form (Eq.\,\,\ref{eq:FENE}). The effective bond width and the strength of the short range repulsion is controlled by the parameter $\eta$. The entire potential is shifted by $(\epsilon+U_{\rm shift})$ in order to match the minimum energy of the non-bonded interactions. The maximum bond extension is limited by the FENE potential, which diverges as $r \rightarrow r_0 \pm R$. Increasing the value of $\eta$ introduces asymmetry to the bonded potential and the energy cost associated with non-optimal bond lengths is increased. In particular, compressed bonds result in high energy penalties as $\eta$ becomes large. The shapes and the widths of the bonded potential at different values of the $\eta$ control parameter are shown in Fig.~\ref{fig:modifiedBondedPotential}. 
%
\begin{figure}
\center
\includegraphics[width=0.75\textwidth]{chapter6Figs/Fig1.eps}\hspace{2pc}%
\caption{\label{fig:modifiedBondedPotential} The modified bonded potential $U_{\mathrm{bonded}}(r)$, modelled by the FENE and LJ interactions. The strength of the Lennard-Jones term is controlled by the bond flexibility parameter $\eta \in [0,1]$.}
\end{figure}
...
\begin{figure}
\center
\includegraphics[width = 1.03\textwidth]{chapter7Figs/combinedMicro.pdf}
...
\begin{figure}
\center
\includegraphics[width = 0.7\textwidth]{chapter7Figs/configurations.pdf}
...
\begin{figure}
\center
\includegraphics[width = 0.7\textwidth]{chapter7Figs/entropyCombined.pdf}
...
\begin{figure}
\center
\includegraphics[width = 0.7\textwidth]{chapter7Figs/scalingCombined.pdf}
