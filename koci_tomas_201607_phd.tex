...
In situations, when the Metropolis sampling of a single canonical ensemble is insufficient, a standard way of increasing the sampling efficiency is to perform the simulation in a conveniently chosen \textit{generalized ensemble}. In parallel tempering, multiple canonical ensembles, with different inverse temperatures $\{\beta_{1} < \beta_{2} < ... < \beta_{N}\}$, are simulated in parallel according to the standard Metropolis scheme.
At judiciously chosen intervals, an exchange of conformations between adjacent temperature threads $i$ and $j$ is proposed. 
