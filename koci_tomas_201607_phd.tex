...
\section{Summary}
%
We have investigated the thermodynamic and structural behavior of elastic homopolymer chains of lengths $N=15,55$, over a range of values of the model parameter $\eta$ controlling the strength of the bonded LJ potential. For small values of this parameter, a freezing transition into an icosahedral phase precedes a solid-solid transition into low-energy states with bihexagonal geometry. The non-optimal bond lengths found in bihexagonal conformations, result in large energetic penalties due to the short-range repulsion between pairs of bonded monomers if $\eta$ is increased. Eventually, only a single icosahedral solid phase remains. The results show that the model is robust over a wide range of values of $\eta$. However, the striking behavior for very small values of $\eta$, illustrates the importance of a careful choice of model parameters.
