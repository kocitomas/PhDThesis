...
In situations, when the Metropolis sampling of a single canonical ensemble is not sufficient, the standard way of increasing sampling efficiency is to perform the simulation in a conveniently chosen \textit{generalized ensemble}\footnote{The microstate probability distribution of a generalized ensemble can be arbitrary and does not have to bear any resemblance to the Boltzmann distribution.}. In parallel tempering, multiple canonical ensembles with different inverse temperatures $\{\beta_{1} < \beta_{2} < ... < \beta_{N}\}$ are simulated in parallel according to the standard Metropolis scheme. In this context, the generalized ensemble is defined as the direct product of the $N$ canonical ensembles, and the partition function is given by
%
\begin{equation}
Z_{\mathrm{gen}}(\beta_{1},\beta_{2},...,\beta_{N}) = \prod_{i=1}^{N} Z_{\mathrm{can}}(\beta_{i}).
\end{equation}
%
...
\begin{equation}
p_{\mu\nu} =  \frac{e^{-\beta_{i}E(\mu)}}{Z_{\mathrm{can}}(\beta_{i})} \frac{e^{-\beta_{j}E(\nu)}}{Z_{\mathrm{can}}(\beta_{j})},
\end{equation}
%
and the acceptance probability for the exchange of conformations is obtained from the equation \ref{eq:MetropolisCriterion}
%
\begin{eqnarray}
\label{eq:replicaExchange}
a\left(\mu \leftrightarrow \nu;\beta _{i}, \beta_{j} \right) &=&
\mathrm{min}\left(1, \frac{e^{-\beta_{i}E(\nu)}e^{-\beta_{j}E(\mu)}}{e^{-\beta_{i}E(\mu)}e^{-\beta_{j}E(\nu)}}\right) \nonumber \\
&=&
...
temperature range which leads to the decrease the autocorrelation time and reduces the likelihood of getting trapped in local energy minima at low temperatures. However, the performance of this method rapidly decreases near first-order transitions, where the joint effect of the entropic suppression of intermediate states and the energetic separation between the ordered and disordered pseudophases, virtually prevents the exchange of configurations between adjacent ensembles.
...
\caption{\label{fig:MGMR}%
Canonical and MGME energy histograms at a strong first-order pseudophase transition. The canonical energy histogram is clearly bimodal, indicating the coexistence of the ordered and disordered pseudophases  while the MGME histograms are all of unimodal shape.}
\end{figure} 
% 
