...
The method for obtaining simplified models by the systematic reduction of the number of degrees of freedom is typically called \text{coarse-graining}. 

In the $6N$-dimensional phase space, the canonical partition function is
\begin{equation}
Z_{\mathrm{can}} = \int \mathcal{DP}\mathcal{DX}e^{-\beta \mathcal{H}(\mathbf{P,X})},
\end{equation}
where
\begin{equation}
\int \mathcal{DP}\mathcal{DX} = C_{\mathrm{N}}\prod_{n = 1}^{N}\iint\frac{d^{3}p_{n}d^{3}x_{n}}{(2 \pi \hbar)^{3}}.
\end{equation}
For the hamiltonian in the standard form
\begin{equation}
\mathcal{H}(\mathbf{P,X}) = \sum_{n = 1}^{N} \frac{\mathbf{p}_{n}^{2}}{2m} + V(\mathbf{X}),
\end{equation}
the momentum integrals factorize and yield
\begin{equation}
\left(\dfrac{m}{2\pi \beta \hbar ^{2}}\right)^{3N/2} = \lambda _{\mathrm{th}}^{-3N}.
\end{equation}
...
where
\begin{equation}
\int \mathcal{DX} = C_{\mathrm{N}}\prod_{n = 1}^{N}\int\frac{d^{3}x_{n}}{\lambda _{\mathrm{th}}^{3}}.
\end{equation}
We introduce the reduced set of coordinates which represent the coarse-grained degrees of freedom
\begin{equation}
\mathbf{Q(X)} = (q_{1}(\mathbf{X}),q_{2}(\mathbf{X}),....,q_{L}(\mathbf{X})).
\end{equation}
The partition function can be written in the new set of coordinates as
\begin{equation}
Z_{\mathrm{can}} = \lambda _{\mathrm{th}}^{L}\int \mathcal{DQ}\int \mathcal{DX}\prod _{l = 1}^{L} \left[\delta (q_{l} - q_{l}^{s}(\mathbf{X}))\right]e^{-\beta V(\mathbf{X})} = \int \mathcal{DQ}e^{-\beta \tilde{V}(\mathbf{X})},
\end{equation}
where $\tilde{V}(\mathbf{X})$ is the effective potential of the coarse-grained model

\begin{equation}
\tilde{V}(\mathbf{X}) = -k_{\mathrm{B}}\,T\,ln \lambda _{\mathrm{th}}^{L}\int \mathcal{DX}\prod_{l = 1}^{L} \left[\delta (q_{l} - q_{l}^{s}(\mathbf{X}))\right]e^{-\beta V(\mathbf{X})} = -k_{\mathrm{B}}\,T\,ln\,Z_{\mathrm{res}}(\mathbf{Q}).
\end{equation}
